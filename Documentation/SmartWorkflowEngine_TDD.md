...
- An extensible library of "actions" (e.g., `create-crm-contact`, `send-sms`, `lookup-order-status`).
...
graph TD
    ...
    subgraph "External Systems"
        CRMApi[<fa:fa-users> CRM API]
        SmsApi[<fa:fa-comment-alt> SMS API]
        SchedulingApi[<fa:fa-calendar-alt> Scheduling API]
    end
    ...
    ActionLibrary -- "4. Execute Action" --> CRMApi
    ActionLibrary -- "5. Execute Action" --> SmsApi
    ActionLibrary -- "6. Execute Action" --> SchedulingApi
```
